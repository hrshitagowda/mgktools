...
from mgktools.hyperparameters.hyperopt import bayesian_optimization, bayesian_optimization_gpr_multi_datasets
from mgktools.exe.args import CommonArgs, KernelArgs, TrainArgs, HyperoptArgs, HyperoptMultiDatasetArgs, EmbeddingArgs
...
def mgk_hyperopt_multi_datasets(arguments=None):
    ...
    for i, data_path in enumerate(args.data_paths):
        ...
        if os.path.exists(dataset_pkl):
            ...
            dataset = Dataset.from_df(df=pd.read_csv(data_path),
                                      ...
                                      n_jobs=args.n_jobs)
            dataset.graph_kernel_type = args.graph_kernel_type
            dataset.save(path=args.save_dir, filename=f'dataset_{i}.pkl', overwrite=False)
    ...
    kernel_config = get_kernel_config(dataset=dataset,
                                      ...
                                      features_hyperparameters_file=args.features_hyperparameters_file,
                                      mgk_hyperparameters_files=args.graph_hyperparameters)
    best_hyperdict, results, hyperdicts = bayesian_optimization_gpr_multi_datasets(save_dir=args.save_dir,
                                                                                   datasets=datasets,
                                                                                   kernel_config=kernel_config,
                                                                                   ...
                                                                                   d_alpha=args.d_alpha,
                                                                                   seed=args.seed)
    pd.DataFrame(results, columns=args.data_paths).to_csv('%s/hyperopt_traj.csv' % args.save_dir, index=False)
