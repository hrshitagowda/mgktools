...
from mgktools.exe.run import mgk_hyperopt, mgk_hyperopt_multi_datasets
...
def test_hyperopt_PureGraph_regression(dataset, split_set, num_splits, metric, graph_hyperparameters, optimize_alpha):
    task = 'regression'
    model = 'gpr'
...
def test_hyperopt_PureGraph_FeaturesAdd_regression(
        dataset, group_reading, features_scaling, metric,
...
def test_hyperopt_Multi_Datasets(features_generator):
    save_dir = '%s/data/_hyperopt_multi_datasets' % CWD
    assert not os.path.exists('%s/hyperparameters_0.json' % save_dir)
    arguments = [
        '--save_dir', save_dir,
        '--data_paths', '%s/data/freesolv.csv' % CWD, '%s/data/bace.csv' % CWD,
        '--pure_columns', 'smiles;smiles',
        '--target_columns', 'freesolv;bace',
        '--tasks_type', 'regression' , 'binary',
        '--alpha', '0.01',
        '--metrics', 'r2', 'roc-auc',
        '--graph_kernel_type', 'graph',
    ...
    if features_generator is not None:
        arguments += [
            ...
            '--features_hyperparameters_min', '0.1',
            '--features_hyperparameters_max', '30.0',
        ]
    mgk_hyperopt_multi_datasets(arguments)
    os.remove('%s/hyperparameters_0.json' % save_dir)
    for i in range(2):
